...
router.post('/procure/contracts', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const contract = cendiaProcureService.addContract({
      ...
      endDate: expirationDate instanceof Date ? expirationDate.toISOString() : expirationDate,
      renewalDate: expirationDate instanceof Date ? expirationDate.toISOString() : expirationDate,
