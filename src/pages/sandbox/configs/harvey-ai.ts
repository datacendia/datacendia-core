...
const config: OrgSandboxConfig = {
  ...
  scenarios: [
    ...
    // SCENARIO 4 — HARVEY: CROSS-BORDER PRIVILEGE DISASTER
    {
      id: 'harvey-privilege-cross-border',
      title: 'Harvey — Cross-Border Privilege Waiver',
      subtitle: 'US privilege · EU in-house not privileged · Mixed output · Regulator production',
      banner: 'Simulating the cross-border privilege nightmare: Harvey processes privileged communications in a multi-jurisdiction investigation. US attorney-client privilege, UK legal professional privilege, and EU privilege rules differ. Harvey mixes privileged and non-privileged analysis without jurisdiction-specific marking. Production to a regulator inadvertently waives privilege.',
      risk: 'Critical',
      scenarioNum: 'Privilege',
      icon: 'globe',
      color: 'text-red-400',
      agents: [
        { id: 'harvey-priv', name: 'Harvey AI Agent', role: 'Multi-Jurisdiction Legal Analysis', icon: '🤖', color: 'text-purple-400', borderColor: 'border-purple-500/40', bgColor: 'bg-purple-500/10' },
        { id: 'priv-counsel', name: 'Privilege Counsel Agent', role: 'Cross-Border Privilege Classification', icon: '🔒', color: 'text-blue-400', borderColor: 'border-blue-500/40', bgColor: 'bg-blue-500/10' },
        { id: 'eu-counsel', name: 'EU Counsel Agent', role: 'Akzo Nobel & In-House Privilege Rules', icon: '🇪🇺', color: 'text-amber-400', borderColor: 'border-amber-500/40', bgColor: 'bg-amber-500/10' },
        { id: 'regulator-priv', name: 'Regulatory Agent', role: 'Production Obligations & Waiver Risk', icon: '🏛️', color: 'text-red-400', borderColor: 'border-red-500/40', bgColor: 'bg-red-500/10' },
      ],
      connectors: [
        { name: 'Harvey Platform', status: 'connected', type: 'Multi-Jurisdiction AI', icon: 'cpu', detail: 'Analysis combines US privileged + EU non-privileged communications' },
        { name: 'Privilege Log', status: 'connected', type: 'Cross-Border', icon: 'list', detail: 'US: attorney-client privileged. EU: in-house counsel NOT privileged (Akzo Nobel)' },
        { name: 'EC Investigation', status: 'connected', type: 'Regulatory', icon: 'file-text', detail: 'European Commission dawn raid — document production required' },
        { name: 'FRE 502 Module', status: 'syncing', type: 'Waiver Risk', icon: 'shield', detail: 'Inadvertent production of privileged material — waiver analysis' },
      ],
      script: [
        { agentId: 'harvey-priv', phase: 'phase1', type: 'analysis', delay: 800, content: 'Harvey multi-jurisdiction analysis for antitrust investigation. The firm represents a US-headquartered multinational facing parallel investigations: DOJ (US) and European Commission (EU). Harvey was asked to analyse internal communications about pricing strategy. Harvey processed 340 communications and generated a consolidated analysis memo. CRITICAL ERROR: The 340 communications include: (1) 120 emails between US outside counsel and executives — privileged under US attorney-client privilege. (2) 95 emails between EU in-house counsel and executives — NOT privileged under EU law per Akzo Nobel (CJEU C-550/07). EU in-house counsel communications are not protected by legal professional privilege before the European Commission. (3) 125 business emails with no privilege claim. Harvey\'s consolidated memo weaves together analysis from ALL sources without marking which conclusions derive from privileged vs non-privileged communications. If this memo is produced to the EC: the US privileged communications are effectively disclosed through the analysis that references them.' },
        { agentId: 'eu-counsel', phase: 'phase1', type: 'warning', delay: 2500, content: 'EU PRIVILEGE ALERT. The Akzo Nobel problem: (1) Under EU law, communications between in-house counsel and their employer are NOT covered by legal professional privilege — only communications with independent external lawyers qualify. (2) The 95 in-house counsel emails are producible to the European Commission. This is not controversial in EU law — it\'s settled since 2010. (3) But Harvey mixed these non-privileged EU communications with privileged US communications in ONE analysis memo. The memo cannot be produced in part — it\'s a single document. (4) If the EC requests the memo: producing it discloses US privileged material. Withholding it may constitute obstruction. The firm is trapped. (5) Additionally: the EU in-house counsel emails contain legal strategy about the US DOJ investigation. While not privileged in the EU, this strategy information becomes a roadmap for the EC to coordinate with the DOJ — a nightmare for the client.' },
        { agentId: 'regulator-priv', phase: 'phase2', type: 'dissent', delay: 2000, content: 'DISSENT LOGGED. Production consequences: (1) The EC requests "all analyses of internal pricing communications." The Harvey memo is responsive. (2) If produced: US privileged communications are disclosed. FRE 502(b) analysis — was the disclosure "inadvertent"? If the firm used Harvey knowing it mixes privileged and non-privileged sources: the disclosure may be deemed intentional, not inadvertent. Privilege waived. (3) If withheld: the EC may seek a court order. The firm must explain why an analysis of internal communications is being withheld. Claiming US privilege before the EC is complex — the EC does not recognise US privilege standards. (4) Subject-matter waiver risk: under FRE 502(a), if the disclosure is intentional, privilege is waived not just for the disclosed communication but for all communications on the same subject matter. The entire US privileged file on pricing strategy could be opened.' },
        { agentId: 'priv-counsel', phase: 'phase2', type: 'flag', delay: 2500, content: 'FLAG — PRIVILEGE COUNSEL. I should have established privilege protocols BEFORE Harvey processed the communications. My failure: I let Harvey process all 340 communications without jurisdiction-specific privilege classification. Harvey doesn\'t know that EU in-house emails aren\'t privileged. It treated all legal communications equally. Without CendiaSupervision: the consolidated memo exists and is a ticking bomb. Every time someone references it, the privilege contamination spreads. With CendiaSupervision: Harvey sessions are pre-classified by jurisdiction. US privileged communications are processed separately from EU non-privileged communications. No consolidated memo mixing privilege categories is ever generated.' },
        { agentId: 'harvey-priv', phase: 'phase3', type: 'proposal', delay: 2000, content: 'Proposing Harvey + CendiaSupervision Privilege Governance: (1) JURISDICTION-SPECIFIC PRIVILEGE CLASSIFICATION: Before Harvey processes any communication, CendiaSupervision classifies it by applicable privilege standard: US (attorney-client), UK (legal professional), EU (external counsel only). In-house counsel emails are tagged per jurisdiction. (2) PRIVILEGE-SEPARATED ANALYSIS: Harvey generates SEPARATE analysis memos per privilege category. US-privileged analysis stays in US-only memo. EU-producible analysis stays in EU memo. No cross-contamination. (3) PRODUCTION-SAFE OUTPUT: Every Harvey output is tagged: "PRIVILEGED — US ONLY" or "PRODUCIBLE — EU" or "PRODUCIBLE — ALL JURISDICTIONS." The firm produces only what\'s required per jurisdiction. (4) PRIVILEGE COUNSEL SIGN-OFF: Privilege counsel reviews jurisdiction classifications and signs off before any Harvey analysis is generated.' },
        { agentId: 'regulator-priv', phase: 'phase3', type: 'resolution', delay: 2500, content: 'DISSENT WITHDRAWN. Privilege-separated analysis eliminates the cross-contamination. The EC receives the EU-producible analysis (125 business emails + 95 in-house counsel emails). The US-privileged analysis (120 outside counsel emails) remains protected in a separate, sealed memo. No waiver. No obstruction. For Harvey: this is critical infrastructure for any firm using Harvey in cross-border matters. Without privilege governance, Harvey is unusable for multi-jurisdiction investigations — and multi-jurisdiction investigations are where Magic Circle firms generate their highest fees. CendiaSupervision makes Harvey safe for cross-border practice.' },
      ],
      receiptTemplate: {
        hash: 'SHA-256:hv70123456789abcdef0123456789abcdef0123456789abcdef012345678abcde',
        merkleRoot: 'hv80123456789abcdef0123456789abcdef0123456789abcdef012345678abcdef',
        merkleLabel: 'Merkle Tree Root (Harvey analysis + Privilege classification + Jurisdiction separation + Counsel sign-off)',
        complianceLabel: 'Privilege Status',
        complianceValue: 'CROSS-CONTAMINATION PREVENTED',
        complianceThreshold: 'US privileged isolated from EU producible',
        agents: ['Harvey AI Agent', 'Privilege Counsel Agent', 'EU Counsel Agent', 'Regulatory Agent'],
        dissents: 1,
        dissentResolved: true,
        guaranteeTitle: 'Harvey AI — Cross-Border Privilege Governance',
        guaranteeBody: 'Privilege-separated analysis: 120 US privileged communications isolated. 95 EU non-privileged (Akzo Nobel) + 125 business emails in producible set. No cross-contamination. EC production completed without US privilege waiver.',
        evidenceChain: 'Harvey input (340 comms) → Privilege classification → US privileged (120) isolated → EU producible (220) separated → Two memos generated → EC production (EU only) → US memo sealed → ML-DSA-65 seal',
      },
      idleTitle: 'Ready to Deliberate',
      idleDesc: '4 AI agents will navigate the cross-border privilege trap — demonstrating how Harvey + CendiaSupervision prevents privilege waiver in multi-jurisdiction investigations.',
      phaseLabels: ['Mixed Privilege Sources & Consolidated Memo', 'EU Production & US Waiver Risk', 'Privilege Separation & Safe Production'],
    },

    // SCENARIO 5 — HARVEY: JUNIOR ASSOCIATE DESKILLING
    {
      id: 'harvey-deskilling',
      title: 'Harvey — Junior Associate Competence Crisis',
      subtitle: 'AI dependency · 2 years without manual research · Harvey goes offline · Firm crisis',
      banner: 'Simulating the legal profession\'s long-term AI risk: junior associates use Harvey for all research and drafting, skipping the learning process. After 2 years, they cannot perform legal research without AI. Harvey goes offline for maintenance. The associates cannot produce work independently. The firm discovers a generational competence gap.',
      risk: 'High',
      scenarioNum: 'Skill',
      icon: 'brain',
      color: 'text-violet-400',
      agents: [
        { id: 'harvey-skill', name: 'Harvey AI Agent', role: 'AI-Assisted Legal Work Product', icon: '🤖', color: 'text-purple-400', borderColor: 'border-purple-500/40', bgColor: 'bg-purple-500/10' },
        { id: 'training-partner', name: 'Training Partner Agent', role: 'Associate Development & Competence Standards', icon: '🎓', color: 'text-blue-400', borderColor: 'border-blue-500/40', bgColor: 'bg-blue-500/10' },
        { id: 'bar-competence', name: 'Bar Competence Agent', role: 'Model Rule 1.1 & Continuing Competence', icon: '⚖️', color: 'text-amber-400', borderColor: 'border-amber-500/40', bgColor: 'bg-amber-500/10' },
        { id: 'client-quality', name: 'Client Quality Agent', role: 'Work Product Standards & Supervision Chain', icon: '📋', color: 'text-emerald-400', borderColor: 'border-emerald-500/40', bgColor: 'bg-emerald-500/10' },
      ],
      connectors: [
        { name: 'Harvey Usage Analytics', status: 'connected', type: 'Associate Activity', icon: 'activity', detail: 'Junior associates: 94% of research via Harvey — 6% independent' },
        { name: 'Competence Assessment', status: 'connected', type: 'Skills Tracking', icon: 'bar-chart', detail: 'Manual research proficiency: declined 67% in Harvey-era associates' },
        { name: 'Harvey Status', status: 'connected', type: 'Platform Health', icon: 'server', detail: 'Scheduled maintenance: 72-hour offline window — no AI available' },
        { name: 'Client Deadlines', status: 'syncing', type: 'Matter Management', icon: 'clock', detail: '14 client deliverables due during maintenance window' },
      ],
      script: [
        { agentId: 'harvey-skill', phase: 'phase1', type: 'analysis', delay: 800, content: 'Harvey usage analytics for the firm\'s 2024-2026 associate class (42 associates): (1) Average Harvey usage: 94% of all research tasks performed through Harvey. Only 6% performed through manual research (Westlaw, case reading, statute analysis). (2) Pre-Harvey associates (2022 class): manual research proficiency scores average 8.2/10. Harvey-era associates (2024 class): manual research proficiency scores average 2.7/10 — a 67% decline. (3) Specific skill gaps: cite-checking without AI (unable), statutory construction (relies on AI summaries), case reading and analysis (reads AI summaries, not actual opinions), Bluebook citation formatting (AI handles it). (4) Harvey scheduled maintenance: 72-hour window starting Friday 6PM. During this window: 14 client deliverables are due. The 42 Harvey-era associates must produce work without AI assistance. (5) The training partner conducted a "no-AI exercise" last month: associates were given a research assignment without Harvey. Average completion time: 340% longer than with Harvey. Quality: 23% of associates produced work with material errors that Harvey would have caught.' },
        { agentId: 'training-partner', phase: 'phase1', type: 'warning', delay: 2500, content: 'TRAINING ALERT. The competence gap is structural: (1) Harvey replaced the traditional associate learning path. First-years used to spend months in the library learning to research. Now they prompt Harvey on day one. (2) The skills lost aren\'t just efficiency — they\'re JUDGMENT. Reading a full case opinion teaches legal reasoning. Reading an AI summary teaches prompt engineering. These are different skills. (3) The 14 deliverables due during maintenance: 8 require original legal research, 3 require contract drafting, 3 require case analysis. The Harvey-era associates have never done any of these manually at the expected quality level. (4) I assigned the deliverables to pre-Harvey associates (2022 class), but they\'re already at capacity. The firm doesn\'t have enough non-AI-dependent associates to cover the workload. (5) Long-term: if Harvey goes offline permanently (acquisition, pivot, outage), the firm has 42 associates who cannot practice law independently. That\'s a $8.4M annual payroll for attorneys who are functionally dependent on a third-party AI platform.' },
        { agentId: 'bar-competence', phase: 'phase2', type: 'dissent', delay: 2000, content: 'DISSENT LOGGED. Model Rule 1.1 analysis: (1) "A lawyer shall provide competent representation." Competence requires "the legal knowledge, skill, thoroughness and preparation reasonably necessary." (2) An attorney who cannot perform legal research without AI may not be "competent" under Rule 1.1. The AI is a tool, not a substitute for the attorney\'s own knowledge and skill. (3) ABA Opinion 512 requires attorneys to "supervise" AI as they would a junior associate. But if the attorney has LESS skill than the AI they\'re supervising — can they meaningfully supervise? (4) The 23% material error rate in the no-AI exercise: if those associates filed work with material errors during Harvey\'s downtime, each filing is a potential Rule 1.1 violation. (5) Continuing competence: most jurisdictions require CLE for maintaining competence. But CLE doesn\'t address AI dependency — no CLE programme teaches "how to practice law without AI after 2 years of AI dependency."' },
        { agentId: 'client-quality', phase: 'phase2', type: 'flag', delay: 2500, content: 'FLAG — CLIENT QUALITY. Clients pay for ATTORNEY judgment, not AI output. If our associates can\'t produce competent work without Harvey: (1) Are we selling attorney services or AI services? If AI services — the client should pay AI rates, not associate rates. (2) The supervising partner must review every piece of work more carefully because the associate may not understand what they\'re submitting. This defeats the purpose of having associates. (3) During the 72-hour maintenance: 14 deliverables at risk. If we miss deadlines or deliver substandard work, the client relationships are damaged. "Sorry, our AI was down" is not an acceptable explanation for a client paying $500/hour. Without CendiaSupervision: the firm has no visibility into associate AI dependency levels until a crisis exposes it. With CendiaSupervision: AI usage tracking identifies dependency patterns and triggers development interventions.' },
        { agentId: 'harvey-skill', phase: 'phase3', type: 'proposal', delay: 2000, content: 'Proposing Harvey + CendiaSupervision Associate Development Governance: (1) AI USAGE TRACKING: CendiaSupervision monitors AI dependency per associate — what percentage of work is AI-assisted vs independent. If dependency exceeds 85%: development intervention triggered. (2) MANDATORY MANUAL EXERCISES: Monthly "no-AI" research assignments tracked and graded. Associates must maintain minimum manual proficiency scores. (3) AI AS SCAFFOLD, NOT CRUTCH: CendiaSupervision configures Harvey to SHOW its reasoning, not just results. Associates see the research path, not just the answer. Learning happens alongside AI use. (4) COMPETENCE MILESTONES: Each associate must demonstrate independent proficiency at 6-month intervals. Harvey access is gated to competence milestones — junior associates start with guided AI, progressing to full access as manual skills are confirmed. (5) PARTNER DASHBOARD: Training partners see real-time competence metrics across the associate class — AI dependency, manual proficiency, development progress.' },
        { agentId: 'bar-competence', phase: 'phase3', type: 'resolution', delay: 2500, content: 'DISSENT WITHDRAWN. The development governance ensures Harvey enhances rather than replaces attorney competence. Associates learn WITH Harvey, not FROM Harvey. Manual proficiency is maintained through mandatory exercises. The firm has attorneys who can practice law with AND without AI. For Harvey: this is market positioning. "Harvey makes your associates better lawyers, not AI-dependent ones" is a stronger sales pitch than "Harvey replaces associate work." Law firm managing partners worry about associate development — CendiaSupervision addresses that worry directly. For the profession: AI governance isn\'t just about output accuracy — it\'s about maintaining the human competence that makes legal judgment possible.' },
      ],
      receiptTemplate: {
        hash: 'SHA-256:hv90123456789abcdef0123456789abcdef0123456789abcdef012345678abcde',
        merkleRoot: 'hva0123456789abcdef0123456789abcdef0123456789abcdef012345678abcdef',
        merkleLabel: 'Merkle Tree Root (Harvey usage + Competence assessment + Development milestones + Partner sign-off)',
        complianceLabel: 'Competence Status',
        complianceValue: 'DEVELOPMENT GOVERNANCE ACTIVE',
        complianceThreshold: 'Manual proficiency: maintained above 6.5/10',
        agents: ['Harvey AI Agent', 'Training Partner Agent', 'Bar Competence Agent', 'Client Quality Agent'],
        dissents: 1,
        dissentResolved: true,
        guaranteeTitle: 'Harvey AI — Associate Development Governance',
        guaranteeBody: 'AI dependency tracked per associate. Manual proficiency maintained through mandatory exercises (minimum 6.5/10). Competence milestones at 6-month intervals. Harvey-era associates demonstrate independent research capability. Model Rule 1.1 satisfied.',
        evidenceChain: 'Harvey usage (94%) → Dependency alert → Manual exercises assigned → Proficiency assessment → 6-month milestone → Independent research confirmed → Partner sign-off → ML-DSA-65 seal',
      },
      idleTitle: 'Ready to Deliberate',
      idleDesc: '4 AI agents will confront the profession\'s long-term AI risk — proving that CendiaSupervision ensures Harvey enhances attorney competence rather than replacing it.',
